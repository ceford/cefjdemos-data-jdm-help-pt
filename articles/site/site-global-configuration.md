...
- **Sessões Compartilhadas** Quando habilitado, a sessão de um usuário é compartilhada entre as seções Frontend e Backend do site. Note que mudar este valor invalidará todas as sessões existentes no site. Isto não está disponível quando a opção *Forçar HTTPS* está definida como *Apenas Administrador*.
